    ...
    <div>
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          {/* Tab Buttons */}
          <div style={{ display: 'flex', gap: '0.5rem', backgroundColor: theme.bgSubtle, padding: '0.3rem', borderRadius: '6px' }}>
            <button
              type="button"
              ...
              style={{
                ...
                color: activeTab === 'coa' ? theme.accentGold : theme.textMuted,
                boxShadow: activeTab === 'coa' ? '0 2px 6px rgba(0,0,0,0.1)' : 'none',
              }}
            >
            ...
            <button
              ...
              style={{
                ...
                color: activeTab === 'journals' ? theme.accentGold : theme.textMuted,
                boxShadow: activeTab === 'journals' ? '0 2px 6px rgba(0,0,0,0.1)' : 'none',
              }}
            >
        ...
        <div style={{ display: 'flex', gap: '1rem', marginTop: '1.4rem', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '220px', position: 'relative' }}>
            <Search size={14} style={{ position: 'absolute', left: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: theme.textDim }} />
            <input
              type="text"
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.82rem' }}>
              <thead>
                <tr style={{ backgroundColor: theme.bgSubtle, borderBottom: `1px solid ${theme.borderLight}`, color: theme.textMuted }}>
                  <th style={{ padding: '0.85rem 1.2rem', fontWeight: 600 }}>Code</th>
                  <th style={{ padding: '0.85rem 1.2rem', fontWeight: 600 }}>Account Name</th>
                  <th style={{ padding: '0.85rem 1.2rem', fontWeight: 600 }}>Type</th>
                  <th style={{ padding: '0.85rem 1.2rem', fontWeight: 600, textAlign: 'right' }}>Current Balance</th>
                </tr>
              </thead>
              <tbody>
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                    <span style={{ fontWeight: 700, color: theme.accentGold, fontSize: '0.9rem' }}>
                      {je.id}
                    </span>
                ...
                <div style={{ fontSize: '0.8rem', color: theme.textMuted, marginBottom: '0.8rem' }}>
                  <strong>Narration / Ref:</strong> {je.ref}
                </div>

                {/* Journal Entry Lines Table */}
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem', textAlign: 'left' }}>
                  <thead>
                    <tr style={{ borderBottom: `1px solid ${theme.borderLight}`, color: theme.textDim }}>
                      <th style={{ padding: '0.4rem 0.6rem', fontWeight: 600 }}>Account</th>
                      <th style={{ padding: '0.4rem 0.6rem', fontWeight: 600, textAlign: 'right' }}>Debit (₹)</th>
                      <th style={{ padding: '0.4rem 0.6rem', fontWeight: 600, textAlign: 'right' }}>Credit (₹)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {je.lines.map((line, idx) => (
                      <tr key={idx} style={{ borderBottom: `1px solid ${theme.borderLight}` }}>
                        <td style={{ padding: '0.45rem 0.6rem', color: theme.textMain }}>
                          <span style={{ color: theme.accentGold, fontWeight: 600, marginRight: '0.5rem' }}>{line.accountCode}</span>
                          {line.accountName}
                        </td>
                        <td style={{ padding: '0.45rem 0.6rem', textAlign: 'right', color: line.debit > 0 ? theme.textMain : theme.textDim, fontWeight: line.debit > 0 ? 600 : 400 }}>
                          {line.debit > 0 ? line.debit.toLocaleString() : '-'}
                        </td>
                        <td style={{ padding: '0.45rem 0.6rem', textAlign: 'right', color: line.credit > 0 ? theme.textMain : theme.textDim, fontWeight: line.credit > 0 ? 600 : 400 }}>
                          {line.credit > 0 ? line.credit.toLocaleString() : '-'}
                        </td>
                      </tr>
                    ))}
                    <tr style={{ fontWeight: 700, color: theme.textMain }}>
                      <td style={{ padding: '0.6rem 0.6rem', textAlign: 'right' }}>Total:</td>
                      <td style={{ padding: '0.6rem 0.6rem', textAlign: 'right', color: theme.accentGold }}>₹{totalDebit.toLocaleString()}</td>
                      <td style={{ padding: '0.6rem 0.6rem', textAlign: 'right', color: theme.accentGold }}>₹{totalCredit.toLocaleString()}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
